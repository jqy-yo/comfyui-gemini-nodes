...
class GeminiVideoCaptioner:
    ...
    def get_caption_with_frames(self, api_key, frames, prompt, model, temperature, max_output_tokens, top_p, top_k,
        ...
        if api_version == "auto":
            ...
            if "gemini-2.5" in model or "models/gemini-2.5" in model:
                api_version = "v1"  # Gemini 2.5 models typically use v1
            else:
                api_version = "v1beta"  # Default to v1beta for most models
            print(f"[GeminiVideoCaptioner] Auto-selected {api_version} API endpoint for {model}")
        else:
            print(f"[GeminiVideoCaptioner] Using user-specified {api_version} API endpoint for {model}")
        ...
        # Special handling for Gemini 2.5 models
        if "gemini-2.5" in model or "models/gemini-2.5" in model:
            # For Gemini 2.5 models, we might need a different payload structure
            payload = {
                "contents": [{
                    "role": "user",
                    "parts": content_parts
                }],
                "generationConfig": {  # Note the camelCase key
            ...
            # Standard payload for older models
            payload = {
                "contents": [{
                    "role": "user",
                    "parts": content_parts
                }],
                "generation_config": {  # Note the snake_case key for older API
                    "temperature": temperature,
                    "maxOutputTokens": max_output_tokens,
                    "topP": top_p,
                    "topK": top_k,
    ...
    def get_caption_with_video_file(self, api_key, video_path, mime_type, prompt, model,
        ...
        if "gemini-2.5" in model or "models/gemini-2.5" in model:
            ...
            # Add seed if provided
            if seed > 0:
                payload["generation_config"]["seed"] = seed
    ...
    def _send_api_request(self, api_url, payload):
        ...
        try:
            ...
            print(f"[DEBUG] Payload structure: {list(payload.keys())}")
            ...
            # Format 2: Direct "content" format (possible in newer API versions)
            ...
            # Format 3: New model formats with nested structures
            ...
            if "promptFeedback" in result and result["promptFeedback"].get("blockReason"):
                ...
                print(f"[GeminiVideoCaptioner] {block_msg}")
                return f"Error: {block_msg}"
            
            # Dump full response for debugging
            print(f"[DEBUG] Unexpected response format. Full response: {json.dumps(result)[:1000]}...")
