...
"""
    ...
    def _log(self, message):
        ...
        formatted_message = f"[GeminiImageGenADV] {timestamp}: {message}"
    ...
    def _call_gemini_api(self, client_instance, model_name_full, contents, gen_config_obj, retry_indefinitely, retry_count=0, max_retries=3, call_id="0"):
        try:
            ...
            response = client_instance.models.generate_content(
                ...
                contents=contents,
                config=gen_config_obj # GeminiImageEditor uses 'config' for GenerateContentConfig
            )

            # Validate response structure (adapted from GeminiImageEditor)
    ...
    async def _generate_single_image_async(self, api_key, model_name_full, prompt_text, input_pil_images: Optional[List[Image.Image]], temperature, max_retries, retry_indefinitely, seed_val, call_id, api_version="auto"):
        try:
            ...
            if api_version != "auto":
                self._log(f"[Call {call_id}] Note: Using google-genai SDK which automatically handles API versions. Requested version: {api_version}")
            ...
            actual_seed = seed_val if seed_val != 0 else random.randint(1, 0x7fffffff)  # INT32 max value
